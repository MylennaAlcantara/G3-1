...
export function picoFaturamentoHoraPDF (dataFinal, dataInicial, NFE, NFCE, valorFilial, valorIdTop, hora, empresa, user){
    ...
    const QuantidadeNFE = hora.reduce((a, b) => a + b.qtd_nfe, 0);
    const QuantidadeNFCE = hora.reduce((a, b) => a + b.qtd_nfe, 0);
    const ValorNFE = hora.reduce((a, b) => a + b.vlr_total_nfe, 0);
    const ValorNFCE = hora.reduce((a, b) => a + b.vlr_total_nfce, 0);
    const QuantidadeVenda = hora.reduce((a, b) => a + b.qtd_vendas, 0);
    const ValorTotal = hora.reduce((a, b) => a + b.vlr_total, 0);
    const TiketMedio = hora.reduce((a, b) => a + b.tiket_medio, 0);
