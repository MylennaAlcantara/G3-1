...
export function picoDeFaturamentoAnoPDF (dataFinal, dataInicial, NFE, NFCE, valorFilial, valorIdTop, ano, empresa, user){
    pdfMake.vfs = pdfFonts.pdfMake.vfs;

    const QuantidadeNFE = ano.reduce((a, b) => a + b.qtd_nfe, 0);
    const QuantidadeNFCE = ano.reduce((a, b) => a + b.qtd_nfe, 0);
    const ValorNFE = ano.reduce((a, b) => a + b.vlr_total_nfe, 0);
    const ValorNFCE = ano.reduce((a, b) => a + b.vlr_total_nfce, 0);
    const QuantidadeVenda = ano.reduce((a, b) => a + b.qtd_vendas, 0);
    const ValorTotal = ano.reduce((a, b) => a + b.vlr_total, 0);
    const TiketMedio = ano.reduce((a, b) => a + b.tiket_medio, 0);
    ...
    const MêsAno = ano.map((data) => {
        return [
            { text: data.mes, fontSize: 8 },
    ...
    const header = [
        {
            text: 'Pico de Faturamento por Meses do ano',
            style: 'subheader',
            fontSize: 15,
            bold: true,
            alignment: 'left',
    ...
    const content = [
        ...
        },
        {
            table: {
                ...
                body: [
                    [
                        { text: 'Mês', fillColor: '#E0E7ED', fontSize: 6.5 },
